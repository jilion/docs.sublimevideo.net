...
Defines the title to be shared. This setting will only be used for social networks that let you share a title and a text (currently used for Facebook).
...
Defines the main text to be shared (Used for Facebook, Twtter and Pinterest).
...
*Allowed values:*
* @auto@ - Depending on the social network, the image will be taken from "Open Graph":#open_graph, the _img_ elements on the page or the _video_ poster.
** Facebook will first check Open Graph tags in your page and will fallback to the DOM when necessary.
** Pinterest will use the _video_ poster frame.
