...
All these settings can be configured via your site's "Players" page on <%= link_to "MySublimeVideo", my_url('sites') %> but can also be overridden on per video basis with the settings described below.

*Important:* When uploading you own logo, please make sure to double the size of the image (@2x) to allow optimal rendering on Retina displays.
...
If you need to override a player default settings for a specific video, you can do it with the following "settings":/beta/settings/usage.
