...
These settings can be set on the @<a>@ element that triggers the "Lightbox":/beta/lightbox to customize its aspect and behavior. Please note that, if you are using our "(c)JavaScript API":/beta/javascript-api/usage to prepare your lightboxes, these settings must be set @before@ preparing them.
