h2. <%= title('Google Analytics add-on') %>
...
Once "subscribed":#subscribe and "configured":#settings, the Google Analytics add-on automatically tracks a "list of events":#tracked-events that will appear in your Google Analytics dashboard under the <%= link_to('"Events" section', '#google_analytics-sidebar-screenshot', class: 'sublime lightbox')%>. Tracked events includes:
...
<a href='<%= my_url('addons/google_analytics?p=standard') %>'>Subscribe to the Google Analytics add-on <span class='icon-arrow'></span></a>

*Note:* Once subscribed, make sure to:

# "Set up your &lt;video&gt;":#setup,
# Enable the Google Analytics integration via your site's Player editor page on <%= link_to "MySublimeVideo", my_url('sites') %>.
...
|        _Sharing pane open_ "^1^":#note-1         | The sharing button has been clicked and the sharing pane has opened. |
(alt). | _Sharing click - Twitter_ "^1^":#note-1   | The Twitter sharing button has been clicked. |
| _Sharing click - Facebook_ "^1^":#note-1         | The Facebook sharing button has been clicked. |
(alt). | _Sharing click - Pinterest_ "^1^":#note-1 | The Pinterest sharing button has been clicked. |
| _Sharing click - Google+_ "^1^":#note-1          | The Google+ sharing button has been clicked. |
(alt). | _Embed pane open_               | The embed button has been clicked and the embed pane has opened. |
|        _Logo click_ "^2^":#note-2                | The on-video logo has been clicked. |

<a id='note-1'></a> ^1^ : Only available if you're subscribed to the "Social Sharing add-on":/addons/sharing.
<a id='note-2'></a> ^2^ : Only available if you're subscribed to the "Your Logo add-on":/addons/on-video-logo.
