...
All the settings listed above can be configured via your site's Player editor page on <%= link_to "MySublimeVideo", my_url('sites') %> but can also be overridden on per video basis with the settings described below.
...
<a href='<%= my_url('addons/logo?p=disabled') %>'>Choose a site and subscribe to the On-Video Logo add-on <span class='icon-arrow'></span></a>
